...
ipcMain.on('audio-chunk', (event, payload) => {
    ...
    if (payload.isTest) {
        ...
        return; // Don't save it to the actual file
    }

    totalChunksReceived++;
    if (totalChunksReceived % 10 === 0) {
        console.log(`[AUDIO MAIN] Received ${totalChunksReceived} chunks so far...`);
    }

    try {
        // payload.data is a standard Array of Uint8 values representing WebM binary data
        const uint8Array = new Uint8Array(payload.data);
        const buffer = Buffer.from(uint8Array);
        audioBuffer.push(buffer);
    } catch (err) {
        console.error("[AUDIO MAIN] Error converting audio chunk buffer:", err);
    }
});
...
async function uploadToS3(filePath, fileName) {
    ...
    const bucketName = process.env.AWS_S3_BUCKET || "syncmind-transcripts";
...
ipcMain.on('stop-audio', async (event) => {
    ...
    // Add WAV header
    const wavBuffer = writeWavHeader(Buffer.from(pcm16.buffer), 48000, 1, 48000 * 2);
...
ipcMain.handle('get-desktop-sources', async (event, options) => {
    // Default to 'screen' if frontend didn't specify, because 'window' capture breaks Audio MediaRecorder on Windows
