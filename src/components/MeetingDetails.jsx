...
const MeetingDetails = ({
  ...
  const toggleTask = async (index) => {
    ...
    const currentActionItems = getActionItems(meeting);
    if (!meeting || currentActionItems.length === 0) return;

    const updatedActionItems = currentActionItems.map((item, itemIndex) =>
      itemIndex === index
    ...
    if (onUpdate) {
      ...
      showToast(
        result?.message ||
          (nextCompleted ? "Action item completed." : "Action item reopened."),
      );
    }
  ...
  return (
    <div className="flex-1 h-full bg-transparent flex flex-col relative transition-colors duration-200 print:h-auto print:block print:bg-white">
      {/* Header */}
      <div
        className={`px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-start sticky top-0 bg-white/30 dark:bg-gray-900/30 backdrop-blur-lg z-10 transition-colors print:static print:bg-white print:border-none print:px-0 print:py-4 ${standalone ? "w-full" : ""}`}
      >
          ...
          <section>
            ...
            <ul className="space-y-4">
              {meeting.key_points.map((point, index) => (
                <li
                  key={index}
                  className="flex gap-4 text-base lg:text-lg text-gray-700 dark:text-gray-300"
                >
                  <span className="text-blue-500 mt-1">•</span>
                  <span>{point}</span>
                </li>
              ...
              {actionItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-start sm:items-center gap-4">
                      <div
                        className={`w-6 h-6 rounded flex items-center justify-center border flex-shrink-0 transition-colors mt-0.5 sm:mt-0
                                                ${
                                                  isCompleted
                                                    ? "bg-green-500 border-green-500 text-white"
                                                    : "border-gray-300 dark:border-gray-600 group-hover:border-blue-500 text-transparent"
                    ...
                    <div className="flex items-center gap-2 pl-10 sm:pl-0">
                      ...
                      />
                      <span
                        className={`text-sm font-medium ${isCompleted ? "text-gray-400" : "text-gray-600 dark:text-gray-300"}`}
                      >
                        {item.owner}
                      </span>
                    </div>
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-gray-800 m-4">
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 rounded-lg transition-colors shadow-sm shadow-blue-600/20"
                >
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-gray-800 m-4">
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-red-400 rounded-lg transition-colors shadow-sm shadow-red-600/20"
                >
